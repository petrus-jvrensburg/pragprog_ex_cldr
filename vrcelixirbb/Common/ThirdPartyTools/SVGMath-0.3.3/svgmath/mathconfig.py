...
class MathConfig(sax.ContentHandler):
        ...
        if name == u"config":
            ...
            try:
                if u"afm" in attributes.keys():
                    fontpath = self.makeAbsolutePath(attributes.get(u"afm"))
                    metric = AFMMetric(fontpath, attributes.get(u"glyph-list"), sys.stderr)
                elif u"ttf" in attributes.keys():
                    fontpath = self.makeAbsolutePath(attributes.get(u"ttf"))
    ...
    def makeAbsolutePath(self, path):
        if path[0] == u".":
            return os.path.join(os.path.dirname(__file__), u"..", path)
        else:
            return path
