...
#DT    print STDERR "Reading data from file: $filename - $x x $y - $type\n";
...
#	and $type ne "PNG") {
	...
	((not $rescale_large_images)) ## DT ## || ($x <= $maxwidth and $y <= $maxheight))
	and $type ne "PNG"
	and (not defined $scale_factor or $scale_factor == 1.0)) {
	
	# No transformation has to be done, keep data as is
#DT	print STDERR "No transformation: $filename - $x x $y\n";
...
#    my $y = $p->height;
  ...
  ##  print STDERR "UTIL FILENAME: $filename\n";
	...
	print "Current is $size\n";
	$quality = 100;
	while (length (MobiPerl::Util::get_gd_image_data ($p, $filename, $quality)) >
	       $maxsize and $quality > 0) {
	    #print "Quality is $quality, size is ";
	    #print length (MobiPerl::Util::get_gd_image_data ($p, $filename, $quality));
	    #print "\n";
	    $quality -= 10;
	}
	if ($quality < 0) {
...
sub fix_pre_tags {
	...
	my $text = $content[0];
	print STDERR "FIX PRE TAGS: $text\n";
