...
<xsl:template match="html">
  <xsl:copy>
    <xsl:apply-templates select="head" mode="force"/> 
...
<xsl:template match="body">
  <xsl:copy>
    ...
    <xsl:apply-templates select="h1" mode="force"/> 
    <xsl:apply-templates select="//div[@ppextract='yes']" mode="force"/>
    <xsl:apply-templates select="//div[@class='copyright']" mode="force"/>
  </xsl:copy>
</xsl:template>

<xsl:template match="span[@class='chapter-number']" mode="force">
...
<xsl:template name="image-link">
  <xsl:variable name="home-page">
    <xsl:apply-templates select="//link[@rel='homepage']" mode="header"/>
  </xsl:variable>
  ...
  <div class="extract-header">
	...
	<img>
	  ...
	  <xsl:attribute name="alt">
	    <xsl:apply-templates select="//meta[@name='book-title']" mode="header"/>
	  </xsl:attribute>
	</img>
      </a>
    </div>
	...
	<xsl:apply-templates select="//meta[@name='book-author']" mode="header"/>.
      </span>
    </p>
...
<xsl:template match="meta[@name='book-title']" mode="header">
  <xsl:value-of select="@content" mode="force"/>
</xsl:template>

<xsl:template match="meta[@name='book-author']" mode="header">
  <xsl:value-of select="@content" mode="force"/>
</xsl:template>

<xsl:template match="link" mode="header">
