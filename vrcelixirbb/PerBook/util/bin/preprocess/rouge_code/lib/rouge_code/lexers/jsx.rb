...
module Rouge
  module Lexers
    class EmbeddedJSX < RegexLexer
      ...
      state :assignment_expression do   # actually a JS expr in {...}
        rule /{(.*?)}/m do |match|
          inside = match.matched[1...-1]
          delegate jsx, inside
        end
      end

      state :root do
        mixin :assignment_expression
        rule /[^<&]+/, Text
        rule /&\S*?;/, Name::Entity
...
#      desc "JSX variant of JavaScript"
#
#      tag 'jsx'
#      filenames '*.jsx'
#      mimetypes 'text/jsx'
...
#
#      def initialize(opts={})
#        @xml_lexer = EmbeddedJSX.new(opts)
#        super(opts)
#      end
#
